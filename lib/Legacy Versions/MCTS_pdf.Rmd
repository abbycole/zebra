...
for(i in names(subtaxa)){
    dates <- names(subtaxa) #timestamp for each observed sample
    abund <-subtaxa[,dates[i]] #abundances for selected timestamps
    mostabund<- tail(sort(abund),10) #vector of 10 most abundant species (their counts, at least)
