...
#### Massaging ####
staxa = taxa[,colSums(taxa)>=20000]
staxa = sweep(taxa,2,colSums(taxa),'/');           # Normalize
staxa = taxa[order(rowMeans(taxa),decreasing=F),]; # Sort by avg. abundance
staxa = taxa[rowMeans(taxa) >= 0.0001,];            # Drop rare taxa (abundance)
staxa = taxa[rowSums(taxa > 0) > 10,];             # Drop rare taxa (prevalence)
...
for (id in unique(map$UserName)){
  subtaxaalpha <- staxa[(colnames(staxa) %in% submap[,"X.SampleID"])]
  # render(input = "lib/MCTS_pdf_mcb.rmd",output_file = paste0('report.', id, '.pdf'),"pdf_document",
  # output_dir = "output/5.31.test2") 
}
